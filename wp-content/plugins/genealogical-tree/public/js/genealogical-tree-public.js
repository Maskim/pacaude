...
var ginge = (function() {
	...
	var loadFam = function(famId, indiId, hilight) {
		...
		//console.log( ' indiId - '+indiId +' famId - '+famId + ' - loadFam run')
...
(function($) {
    ...
    jQuery(window).on('load', function() {
        jQuery(document).ready(function() {
            ...
            function tree(id, setting) {
                var cont = $('li[data-id="' + id + '"]');
                jQuery.ajax({
                    url: gtObj.gt_rest_url + "genealogical-tree/v1/member/indi_" + id + ".js",
                    cache: true,
                    dataType: "json"
                    ...
                    var temp = generate_template(ind.root.indi, setting, '')
                    ...
                    if(ind.root.indi.fam) {
                        $.each(ind.root.indi.fam, function(index, fam) {
                            jQuery.ajax({
                                url: gtObj.gt_rest_url + "genealogical-tree/v1/family/fam_" + fam + ".js",
                                cache: true,
                                dataType: "json"
                            }).done(function(fam) {
                                ...
                                var temp = generate_template(partner, setting, 'spo')
                                cont.append(temp);
                            })
                        });
                        $.each(ind.root.indi.fam, function(index, fam) {
                            jQuery.ajax({
                                ...
                                if(fam.root.fam.children) {
                                    cont.parent().append('<ul class="chills"></ul>');
                                    cont.removeClass('no-chills');
                                    $.each(fam.root.fam.children, function(index, children) {
                                        cont.parent().find('.chills').append('<li data-id="' + children.ref + '"><ul class="fam no-chills"></ul></li>');
                                        tree(children.ref, setting);
                                    })
                                }
                            })
                        });
                ...
                for(var i = 0; i < uls.length; i++) {
                    ...
                    if(childrens.length > 1) {
                        var heights = [];
                        for(var j = 0; j < childrens.length; j++) {
                            var height = $(childrens[j]).find('div.ind').first().height();
                            heights.push(height)
                        }
                        for(var k = 0; k < childrens.length; k++) {
                            //$(childrens[k]).find('div.ind').first().height(Math.max.apply(Math, heights));
                        }
                    }
                }
            }

            function generate_template(ind, setting, cl) {
                var temp = '';
                var image = '';
                console.log(setting.image.show);
                if(setting.image.show) {
                    if(ind.image !== false) {
                        image += '\
						<div class="gt-ind-image">\
							<img src=' + ind.image + '">\
						</div>';
                    } else {
                        image += '\
						<div class="gt-ind-image">\
							<img src="' + gtObj.gt_dir_url + 'public/img/ava-' + ind.sex + '.jpg">\
						</div>';
                    }
                }
                var birt = '';
                ...
                var birtO = ind.birt;
                var deatO = ind.deat;
                if(setting.birt.show === true && birtO !== null) {
                    birt = birtO;
                }
                if(setting.deat.show === true && deatO !== null) {
                    deat = deatO;
                }
                if(setting.deat.show === true && setting.deat.show === true) {
                    if(birtO !== null) {
                        birt = birtO;
                    }
                    if(deatO !== null) {
                ...
                if(setting.deat.show === true || setting.deat.show === true) {
                    birt_deat += '\
					<div class="gt-ind-birt-deat">\
						' + birt + ' ' + sep + ' ' + deat + '\
					</div>';
                }
                var gender = '';
                if(setting.gender.show === true) {
                    if(ind.sex) {
                        gender += '(' + ind.sex + ')';
                    }
                }
                temp += '\
				<li class="child root">\
					<div class="ind ' + cl + ' ' + ind.sex + '" data-id="' + ind.id + '">\
						' + image + '\
						<div class="gt-ind-details">\
							<div class="gt-ind-name">\
							' + ind.full_name + ' ' + gender + '\
							</div>\
							' + birt_deat + '\
						</div>\
					</div>\
