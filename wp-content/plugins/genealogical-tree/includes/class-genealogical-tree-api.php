...
class Genealogical_Tree_Api
{
    ...
    {
        register_rest_route( 'genealogical-tree/v1', '/member/indi/(?P<id>\\d+)', array(
            'methods'  => 'GET',
    ...
    {
        register_rest_route( 'genealogical-tree/v1', '/family/fam/(?P<id>\\d+)', array(
            'methods'  => 'GET',
    ...
    public function get_fam_ref_id( $object_id )
    {
        ...
        if ( $query->posts ) {
            foreach ( $query->posts as $key => $family ) {
                ...
                $spouse = get_post_meta( $family->ID, 'spouse', true );
    ...
    public function route_member_ind( $obj )
    {
        ...
        $tree_event = $this->getTreeEvents( $object_id );
        if ( $tree_event ) {
        ...
        $data = (object) array(
            ...
            'id'        => $object_id,
            'sex'       => $sex,
            'full_name' => ( get_post_meta( $object_id, 'full_name', true ) ? get_post_meta( $object_id, 'full_name', true ) : 'Unknown' ),
            'name'      => (object) array(
            ...
            'fam'       => $fam,
            'event'     => $event,
            'image'     => $this->f_image( $object_id ),
            'birt'      => $this->find_birt_deat_by_id( $object_id, 'birt' ),
            'deat'      => $this->find_birt_deat_by_id( $object_id, 'deat' ),
        ),
        ),
        );
        //print_r($data);
     ...
     * @since    1.0.0
     */
    public function find_birt_deat_by_id( $member, $type )
    {
                    ...
                    if ( $key === 'birt' ) {
                        if ( current( $event_single ) ) {
                            if ( current( $event_single )['date'] ) {
                                return current( $event_single )['date'];
                            }
                        }
                    }
                }
    ...
    public function route_family_fam( $obj )
    {
        ...
        if ( $children ) {
            foreach ( $children as $key => $child ) {
                ...
                $childs[$child] = array(
                    ...
                    'full_name' => ( get_post_meta( $child, 'full_name', true ) ? get_post_meta( $child, 'full_name', true ) : 'Unknown' ),
                    'name'      => (object) array(
        ...
        $husb_name = ( get_post_meta( $husb, 'full_name', true ) ? get_post_meta( $husb, 'full_name', true ) : 'Unknown' );
        $husbo = (object) array(
            'event'     => $husb_event,
            'image'     => $this->f_image( $husb ),
            'full_name' => ( get_post_meta( $husb, 'full_name', true ) ? get_post_meta( $husb, 'full_name', true ) : 'Unknown' ),
            'name'      => (object) array(
            'first' => $husb_name,
            'last'  => '',
        ),
            'ref'       => $husb,
            'birt'      => $this->find_birt_deat_by_id( $husb, 'birt' ),
            'deat'      => $this->find_birt_deat_by_id( $husb, 'deat' ),
            'sex'       => 'M',
        );
        ...
        $wifeo = (object) array(
            ...
            'full_name' => ( get_post_meta( $wife, 'full_name', true ) ? get_post_meta( $wife, 'full_name', true ) : 'Unknown' ),
            'name'      => (object) array(
            'first' => $wife_name,
            'last'  => '',
        ),
            'ref'       => $wife,
            'sex'       => 'F',
            'birt'      => $this->find_birt_deat_by_id( $wife, 'birt' ),
            'deat'      => $this->find_birt_deat_by_id( $wife, 'deat' ),
        );
        $return = (object) array(
            'root' => (object) array(
            'header' => (object) array(
