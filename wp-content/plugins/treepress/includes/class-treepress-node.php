...
class Treepress_Node {
	...
	public function get_html($the_family) {
		...
		$html .= '</td>';
		$html .= '<td style="vertical-align:bottom" width="33.333333%">'.__('Born:', 'treepress').' '.$this->born.'</td>';
		if (!empty($this->died) && strlen($this->died) > 1) {
			$html .= '<td style="vertical-align:bottom" width="33.333333%">'.__('Died:', 'treepress').' '.	$this->died.'</td>';
		} else {
			$html .= '<td></td>';
		}
	$html .= '</tr>';
	...
	else
	{
		...
		{
			$html .= '<tr><td colspan="4">'.__($plural_son_termonology.': ', 'treepress');
		}
			else
		{
		$html .= '<tr><td colspan="4">'.__($son_termonology.': ', 'treepress');
		}
